"""
4CH Heart Segmentation Model Worker
A FastAPI service that executes the 4-chamber heart segmentation model.

Usage:
    python -m serve.cine_4ch_seg_worker --gpu 0 --port 21011 --no-register
"""
import sys
import os
...
MODEL_SRC_DIR = os.path.join(SRC_DIR, "CINE_4CH_SEG")
...
# Import from src/CINE_4CH_SEG/infer/
...
try:
    ...
    # Fallback definitions if MMedAgent is not available
...
logger = build_logger("cine_4ch_seg_worker", os.path.join("workers", "cine_4ch_seg.log"))
global_counter = 0
...
class HeartSeg4CHWorker:
    """
    Worker class for 4-chamber heart segmentation.
    ...
    def __init__(
        ...
        logger.info("Loading 4CH heart segmentation model...")
        self._load_model()
    ...
    def _load_model(self):
        ...
        model_seg = CtAbdomenSegDYModel(
            ...
            model_DY_crop_f=self.model_config["model_file_DY_secondL"],
            model_DY_crop_r=self.model_config["model_file_DY_secondR"],
            network_DY_f=self.model_config["network_file_DY_first"],
            network_DY_crop_f=self.model_config["network_file_DY_secondL"],
            network_DY_crop_r=self.model_config["network_file_DY_secondR"],
    ...
    def save_segmentation_image(self, hu_volume: np.ndarray, seg_mask: np.ndarray, 
        ...
        try:
            ...
            colors = {
                ...
                3: [0, 0, 255, 128],      # Blue - RV cavity
                4: [255, 255, 0, 128],    # Yellow - LA cavity
                5: [255, 0, 255, 128],    # Magenta - RA cavity
                6: [0, 255, 255, 128],    # Cyan - Other
            }
...
# FastAPI Application
app = FastAPI(title="4CH Heart Segmentation Service")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="4CH Heart Segmentation Worker")
    ...
    # Model paths - use MODEL_SRC_DIR for relative paths (src/4CH_CINE_SEG/cmr_heart_models)
    ...
    parser.add_argument(
        "--network-file-DY-secondL",
        type=str,
        default=os.path.join(MODEL_BASE_DIR, "train/config/seg_mrdy_stage2L.py"),
    )
    parser.add_argument(
        "--network-file-DY-secondR",
        type=str,
        default=os.path.join(MODEL_BASE_DIR, "train/config/seg_mrdy_stage2R.py"),
    )
    parser.add_argument(
        "--config-file",
        type=str,
    ...
    model_config = {
        ...
        "model_file_DY_secondL": args.model_file_DY_secondL,
        "model_file_DY_secondR": args.model_file_DY_secondR,
        "network_file_DY_first": args.network_file_DY_first,
        "network_file_DY_secondL": args.network_file_DY_secondL,
        "network_file_DY_secondR": args.network_file_DY_secondR,
        "config_file": args.config_file,
    }
    
    # Create worker
    worker = HeartSeg4CHWorker(
