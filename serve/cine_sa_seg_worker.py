"""
SA (Short-Axis) CINE Heart Segmentation Model Worker
A FastAPI service that executes the short-axis CINE heart segmentation model.

Usage:
    python -m serve.cine_sa_seg_worker --gpu 0 --port 21012 --no-register
"""
import sys
import os
...
MODEL_SRC_DIR = os.path.join(SRC_DIR, "CINE_SA_SEG")
...
# Import from src/CINE_SA_SEG/infer/
...
logger = build_logger("cine_sa_seg_worker", os.path.join("workers", "cine_sa_seg.log"))
global_counter = 0
...
class HeartSegSAWorker:
    """
    Worker class for Short-Axis CINE heart segmentation.
    ...
    def __init__(
        ...
        logger.info("Loading SA CINE heart segmentation model...")
        self._load_model()
    ...
    def save_segmentation_image(self, hu_volume: np.ndarray, seg_mask: np.ndarray, 
        ...
        try:
            ...
            # Define colors for different labels (RGBA) - SA has 3 labels
            colors = {
...
# FastAPI Application
app = FastAPI(title="SA CINE Heart Segmentation Service")
...
async def model_details(request: Request):
    ...
    return {
        ...
        "task": "SA (Short-Axis) CINE Heart Segmentation",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SA CINE Heart Segmentation Worker")
    
    # Server configuration
    parser.add_argument("--host", type=str, default="0.0.0.0")
    parser.add_argument("--port", type=int, default=21012)
    parser.add_argument("--worker-address", type=str, default="http://localhost:21012")
    ...
    parser.add_argument(
        "--model-file-DY-first",
        type=str,
        default=os.path.join(WEIGHTS_DIR, "cine_seg_first_SA/latest.pth"),
    )
    parser.add_argument(
        "--model-file-DY-second",
        type=str,
        default=os.path.join(WEIGHTS_DIR, "cine_seg_second_SA/latest.pth"),
    )
    parser.add_argument(
