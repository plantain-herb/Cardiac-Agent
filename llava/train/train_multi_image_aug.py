...
from llava.train.llava_trainer_multi_image_aug import LLaVATrainer
    ...
    for source in sources:
        for sentence in source:
            if DEFAULT_IMAGE_TOKEN in sentence['value']:
                # Remove all existing image tokens first
                sentence['value'] = sentence['value'].replace(DEFAULT_IMAGE_TOKEN, '').strip()
                # Add the correct number of image tokens at the beginning
                image_tokens = (DEFAULT_IMAGE_TOKEN + '\n') * num_images
                sentence['value'] = image_tokens + sentence['value']
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            if not image_files or len(image_files) == 0:
                raise ValueError(f"Empty image_file for data sample {i}. image_file: {image_file}")
            ...
            for img_file in image_files:
                image = self.load_image(img_file, image_folder)
            ...
            if len(images) == 0:
                raise ValueError(f"Empty image list for data sample {i}. image_file: {image_file}, image_files: {image_files}")
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'images' in instances[0]:
            ...
            # Handle multi-image case: each instance may have multiple images
            # images[i] can be either (C, H, W) for single image or (N, C, H, W) for multiple images
            new_images = []
            for img in images:
                if img is None:
                    ...
                    new_images.append(img)
                else:  # Multiple images (N, C, H, W) - flatten to list
                    for j in range(img.shape[0]):
                        new_images.append(img[j])
            
            # Try to stack if all images have the same shape
            if all(x is not None and x.shape == new_images[0].shape for x in new_images):
                batch['images'] = torch.stack(new_images)
            else:
