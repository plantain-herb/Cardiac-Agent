...
class CtAbdomenSegDYPredictor:
    ...
    def DY_predict(
        ...
        DY_mask1, DY_mask2, DY_mask3 = self._predict(
    ...
    def _predict(
        ...
        DY_mask1, DY_mask2, DY_mask3 = self._get_DY_mask(
    ...
    def _get_phy_crop_box(self, vol_shape, spacing_zyx, heart_mask, config):
        ...
        roi_bbox = self._get_bbox(
            heart_mask, shift_upper=[30, 30, 30], shift_lower=[30, 30, 30]
        )
    ...
    def _get_DY_mask(
        ...
        return (heat_map1 > 0.65).astype(np.uint8), (heat_map2 > 0.55).astype(np.uint8), (heat_map3 > 0.5).astype(np.uint8)
