...
save_file = "SA_heart_dice.lst"
ours_path = '/home/qutaiping/nas/ori_data/AZHC/dul/del/SA/test/pred2'
lb_path = '/home/qutaiping/nas/ori_data/AZHC/dul/del/SA/test/seg'
for pid in os.listdir(lb_path):
    pid_ = pid.split('.nii.gz')[0]
    ours = sitk.GetArrayFromImage(sitk.ReadImage(os.path.join(ours_path, pid_ + '-seg.nii.gz')))
    lb = sitk.GetArrayFromImage(sitk.ReadImage(os.path.join(lb_path, pid)))
    # color_table = {0:0, 1:1, 2:2, 3:2, 4:3, 5:4, 6:5, 7:6}
    # for k, v in color_table.items():
    #     lb[lb == k] = v
    nocal = ours.shape[0]//10
    pred = ours[nocal:-nocal, :, :]
    gt = lb[nocal:-nocal, :, :]
    dice = calculate_all_dice(pred, gt, 3)
