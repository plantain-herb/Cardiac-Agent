...
@NETWORKS.register_module()
class Seg_Network_Heart_multi(nn.Module):
    def __init__(
        self, backbone, head, apply_sync_batchnorm=False, pipeline=[], train_cfg=None, test_cfg=None
    ):
        super(Seg_Network_Heart_multi, self).__init__()
    ...
    def forward_test(self, img):
        # TODO: 根据需求适配，python custom/utils/save_torchscript.py保存静态图时使用
        outs = self.backbone(img)
        pred_bin_1, pred_bin_2, pred_bin_3 = self.head(outs)
        pred_bin_1 = torch.sigmoid(pred_bin_1)
        pred_bin_2 = torch.sigmoid(pred_bin_2)
        pred_bin_3 = torch.sigmoid(pred_bin_3)
