...
def parse_args():
    ...
    parser.add_argument('--tgt_path', type=str,
                        default="/home/qutaiping/nas/processed_data/processed_data_LGE_SA_first_stage/")
...
def gen_single_data(info):
    try:
        ...
        if vol.shape != seg.shape:
            print(f'shape :{vol.shape}!={seg.shape}, {pid}')
            return None
...
if __name__ == '__main__':
    ...
    for seg_file in sorted(os.listdir(src_seg_dir)):
        if '_sa_seg.nii.gz' not in seg_file:
            print(f'heart seg file loss:{seg_file}')
            continue
