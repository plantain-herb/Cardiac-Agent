...
def parse_args():
    ...
    parser.add_argument('--src_path', type=str, default='/home/qutaiping/nas/ori_data/LGE_SA/LGE_SA/train')
    parser.add_argument('--tgt_path', type=str,
                        default="/home/qutaiping/nas/processed_data/processed_data_LGE_SA_second_stage_2/")
...
def singe_generate_data(inputs):
    ...
    heart_range_crop = get_seg_max_box(heart_mask > 0, padding=10)
    zmin, ymin, xmin, zmax, ymax, xmax = heart_range_crop
    # print(heart_range_crop, pid)
...
if __name__ == "__main__":
    ...
    src_heart_dir = os.path.join(src_dir, 'seg')
    for seg_file in sorted(os.listdir(src_seg_dir)):
        # train_pred_seg = os.path.join(train_data_path, seg_file)
        pid = seg_file.replace("_sa_seg.nii.gz", "")
        vol_dir = os.path.join(src_dcm_dir, pid + "_sa_image.nii.gz")
        seg_file = os.path.join(src_seg_dir, pid + "_sa_seg.nii.gz")
        heart_path = os.path.join(src_heart_dir, pid + "_sa_seg.nii.gz")
        info = [vol_dir, seg_file, heart_path, tgt_dir, pid]
