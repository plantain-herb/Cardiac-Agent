...
class CtAbdomenSegDYConfig:
    def __init__(self, network_DY_f, network_DY_crop_f, config: Dict):
...
class CtAbdomenSegDYModel:
    def __init__(self, model_DY_f: IO, model_DY_crop_f: IO, network_DY_f, network_DY_crop_f, config_f):
        # TODO: 模型文件定制
...
class CtAbdomenSegDYPredictor:
    def __init__(self, gpu: int, model: CtAbdomenSegDYModel):
        ...
        if self.model.config_f is not None:
            if isinstance(self.model.config_f, str):
                # print(os.getcwd(),2222)
                
                with open(self.model.config_f, "r") as config_f:
                    self.config = CtAbdomenSegDYConfig(
                        self.model.network_DY_f, self.model.network_DY_crop_f, yaml.safe_load(config_f)
                    )
            else:
                self.config = CtAbdomenSegDYConfig(
    ...
    def DY_predict(
        ...
        DY_mask1, DY_mask2 = self._predict(
        ...
        # print(np.sum(DY_mask1), np.sum(DY_mask2))
        DY_mask = DY_mask1 + 2 * DY_mask2
        DY_mask[DY_mask > 2] = 1

        # print(np.unique(DY_mask))
        # DY_mask = self._postprocess_DY(DY_mask_heatmap, heart_mask)
        return DY_mask
    ...
    def _predict(
        ...
        DY_mask1, DY_mask2= self._get_DY_mask(
            hu_volume, heart_mask, src_spacing, pyramid
        )
        return DY_mask1, DY_mask2
    ...
    def _postprocess_DY(self, DY_mask):
        ...
        # def get_max_conn_2d(mask):
        #     z_size, y_size, x_size = mask.shape
        #     result_mask = np.zeros_like(mask, dtype=np.uint8)
            
        #     for z in range(z_size):
        #         slice_2d = mask[z, :, :].astype("uint8")
                
        #         if np.sum(slice_2d) == 0:
        #             continue
                
        #         bin_mask_new, num = cc3d.connected_components(
        #             (slice_2d > 0).astype("uint8"), 
        #             return_N=True, 
        #             connectivity=4 
        #         )
                
        #         if num == 1 or num == 0:
        #             result_slice = bin_mask_new.astype("uint8")
        #         else:
        #             labels, counts = np.unique(bin_mask_new, return_counts=True)
        #             labels = labels.tolist()
        #             labels = sorted(labels, key=lambda x: counts[labels.index(x)], reverse=True)
        #             result_slice = (bin_mask_new == labels[1]).astype("uint8")
                
        #         result_mask[z, :, :] = result_slice
            
        #     return result_mask

        # DY_mask_heatmap = DY_mask
        # DY_mask = (DY_mask_heatmap > 0.6).astype(np.uint8)
    ...
    def _get_DY_mask(
        ...
        with torch.no_grad(), torch.cuda.device(self.gpu):  # type: ignore
            for idx, (data, start_point) in enumerate(
                ...
                pred_seg1, pred_seg2 = self.net_DY_crop.forward_test(data)
                ...
                bin_crop_array1, bin_crop_array2 = self._crop_back(
                    pred_seg1.to(device),
                    pred_seg2.to(device),
                    p_s,
                    start_point,
        ...
        return (heat_map1 > 0.7).astype(np.uint8), (heat_map2 > 0.5).astype(np.uint8)
