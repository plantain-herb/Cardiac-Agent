...
if __name__ == "__main__":
    ...
    for k in ["train", "val"]:
        for key in ["root", "dst_list_file"]:
            if key in cfg.data[k] and "{fold}" in cfg.data[k][key]:
                cfg.data[k][key] = cfg.data[k][key].replace("{fold}", str(args.fold))
    if hasattr(cfg, "work_dir") and "{fold}" in str(cfg.work_dir):
        cfg.work_dir = cfg.work_dir.replace("{fold}", str(args.fold))
