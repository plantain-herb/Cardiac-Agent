...
def parse_args():
    ...
    parser.add_argument(
        "--src_dicom_dir", type=str, default="/home/qutaiping/nas/ori_data/diagnosis_first_refine"
        )
    parser.add_argument(
        "--src_dicom_val_dir", type=str, default="/home/qutaiping/nas/ori_data/diagnosis_first/val_cropped"
        )
    parser.add_argument(
        "--src_labels_dir", type=str, default="/home/qutaiping/nas/zhaocan/heart_diagnosis"
    )
    parser.add_argument(
        "--tgt_dir", type=str, default="/home/qutaiping/nas/processed_data/processed_data_diag_first_refine"
    )
    parser.add_argument(
        "--tgt_val_dir", type=str, default="/home/qutaiping/nas/processed_data/processed_data_diag_first_test/val"
    )
...
def gen_single_data(info):
    ...
    for dcm_path in dcm_files:
        ...
        #     seg_array, _, _ = load_scans(seg_path)
        #     seg_arrays.append(seg_array.astype(np.uint8))
        # else:
        #     print(f"Warning: {seg_path} not found, using zero mask")
        #     seg_arrays.append(np.zeros_like(dcm_array, dtype=np.uint8))

    np.savez_compressed(
        save_file,
        dcm1=dcm_arrays[0],    # 2ch image
        dcm2=dcm_arrays[1],    # 4ch image
        dcm3=dcm_arrays[2],    # sa image
        # seg_2ch=seg_arrays[0], # 2ch mask
        # seg_4ch=seg_arrays[1], # 4ch mask
        # seg_sa=seg_arrays[2],  # sa mask
        gt=gt,
        spacing=spacings,
    )
...
if __name__ == "__main__":
    ...
    excel_file = os.path.join(src_labels_dir, "diag_first_data.csv")
    ...
    for id in os.listdir(src_dcm_dir):
        ...
        dcm_files = [os.path.join(id_path, f) for f in os.listdir(id_path) if f.endswith('_image.nii.gz')]
