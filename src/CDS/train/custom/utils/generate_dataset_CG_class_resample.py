...
def parse_args():
    ...
    parser.add_argument(
        "--src_dicom_dir", type=str, default="/home/taiping-qu/code/mr_heart_seg_thin/train/train_data/ori_data/JFJ_train_resample2"
        )
    parser.add_argument(
        "--src_dicom_val_dir", type=str, default="/home/taiping-qu/code/mr_heart_seg_thin/train/train_data/ori_data/JFJ_test_resample2"
        )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="/home/taiping-qu/code/mr_heart_seg_thin/train/train_data/ori_data/JFJ_seg_resample2"
    )

    parser.add_argument(
        "--src_flow_mask_path", type=str,
        default="/home/taiping-qu/code/mr_heart_seg_thin/train/train_data/ori_data/JFJ_flow2"
    )

    parser.add_argument(
        "--tgt_dir", type=str, default="/home/taiping-qu/code/mr_heart_seg_thin/train/train_data/processed_data_JFJ_class_resample_addflow4"
    )
    parser.add_argument(
        "--tgt_val_dir", type=str, default="/home/taiping-qu/code/mr_heart_seg_thin/train/train_data/processed_data_JFJ_class_resample_addflow4/val"
    )
...
def gen_single_data(info):
    idx, pid, dcm_file, infar_file, flow_file, save_file, gt = info
    # try:
    dcm_array, _, _ = load_scans(dcm_file)
    dcm_shape = dcm_array.shape
    infar_array, _, _ = load_nii(infar_file)
    flow_array, _, _ = load_nii(flow_file)
    flow_array = abs(flow_array)
    ...
    print(bbox[1]-bbox[0], bbox[3]- bbox[2], bbox[5] - bbox[4], np.sum(flow_array[flow_array < 0]))
...
if __name__ == "__main__":
    ...
    src_flow_dir = os.path.join(os.getcwd(),args.src_flow_mask_path)
    excel_file = os.path.join(src_labels_dir, "301_train2.xlsx")
    ...
    for i in range(len(src_path_arr)):
        for pid in os.listdir(src_path_arr[i]):
            ...
            flow_file = os.path.join(src_flow_dir, pid + ".nii.gz")
            dcm_file = os.path.join(src_path_arr[i], pid + ".nii.gz")
            save_file = os.path.join(tgt_path_arr[i], f"{pid}.npz")
            gt = [df.loc[pid, "LABEL"]]
            info = [idx, pid, dcm_file, infar_file, flow_file, save_file, gt]
            idx += 1
