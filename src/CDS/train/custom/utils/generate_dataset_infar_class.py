...
def parse_args():
    ...
    parser.add_argument(
        "--src_dicom_dir", type=str, default="/home/taiping-qu/code/mr_heart_seg_thin/train/train_data/ori_data/infar_nii_class_resample"
        )
    parser.add_argument(
        "--src_dicom_val_dir", type=str, default="/home/taiping-qu/code/mr_heart_seg_thin/train/train_data/ori_data/infar_nii_class_resample"
        )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="/home/taiping-qu/code/mr_heart_seg_thin/train/train_data/ori_data/infar_seg_class_resample"
    )

    parser.add_argument(
        "--src_flow_mask_path", type=str,
        default="/home/taiping-qu/code/mr_heart_seg_thin/train/train_data/ori_data/infar_nii_flow"
    )

    parser.add_argument(
        "--tgt_dir", type=str, default="/home/taiping-qu/code/mr_heart_seg_thin/train/train_data/processed_data_infar_class_resample_val"
    )
    parser.add_argument(
        "--tgt_val_dir", type=str, default="/home/taiping-qu/code/mr_heart_seg_thin/train/train_data/processed_data_infar_class_resample_val/val"
    )
...
def gen_single_data(info):
    idx, pid, dcm_file, infar_file, flow_file, save_file, gta, timea, gtb, timeb = info
    try:
...
if __name__ == "__main__":
    ...
    excel_file = os.path.join(src_labels_dir, "infar_class_train.xlsx")
    ...
    for i in range(len(src_path_arr)):
        for pid in os.listdir(src_path_arr[i]):
            ...
            gta = [df.loc[pid, "MACEA"]]
            timea = [df.loc[pid, "TIMEA"]]
            gtb = [df.loc[pid, "MACEB"]]
            timeb = [df.loc[pid, "TIMEB"]]
            info = [idx, pid, dcm_file, infar_file, flow_file, save_file, gta, timea, gtb, timeb]
            idx += 1
