...
def parse_args():
    ...
    parser.add_argument('--src_path', type=str, default='/home/taiping-qu/code/mr_heart_seg_thin/train/train_data/ori_data')
    parser.add_argument('--tgt_path', type=str,
                        default="/home/taiping-qu/code/mr_heart_seg_thin/train/train_data/processed_data_second_stage2")
...
def gen_single_data(info):
    try:
        ...
        # train_pred_img, _ = load_scans(train_pred_seg)
        # train_pred_mask = sitk.GetArrayFromImage(train_pred_img)
        # train_pred_mask = train_pred_mask.astype(np.uint8)
        ...
        heart_range_crop = get_seg_max_box(seg > 0, padding=20)
        ...
        seg[seg == 205] = 6
        seg[seg == 244] = 7
        # print(np.unique(seg),pid)
        #train_pred_mask = train_pred_mask[zmin: zmax, ymin: ymax, xmin: xmax]
        ...
        # fp  = seg - train_pred_mask
        # fp_point_que = np.argwhere(fp > 0)
        # fp_point_jiayang = np.argwhere(fp < 0)
        ...
        np.savez_compressed(
            save_file,
            vol=vol,
            mask=(seg).astype("uint8"),
            bbox=sample_region,
            src_spacing=spacing_vol,
            data_type=data_type,
            tp_points=points,
            # win_dic=dic,
            # fp_point_queshi=fp_point_que,
            # fp_point_jiayang =fp_point_jiayang
...
if __name__ == '__main__':
    ...
    data_lst = []
    # for cs in ct_set:
    #train_data_path = "/media/tx-deepocean/Data/code/heart_segment/ct_abdomen_seg_heart/example/data/train_data_96"
    src_dcm_dir = os.path.join(src_dir, 'img')
    src_seg_dir = os.path.join(src_dir, 'seg')
    for seg_file in sorted(os.listdir(src_seg_dir)):
        if '_label.nii.gz' not in seg_file:
            print(f'abnormal seg file :{seg_file}')
            continue
        #train_pred_seg = os.path.join(train_data_path, seg_file)
        pid = seg_file.replace("_label.nii.gz", "")
        vol_dir = os.path.join(src_dcm_dir, pid + "_image.nii.gz")
