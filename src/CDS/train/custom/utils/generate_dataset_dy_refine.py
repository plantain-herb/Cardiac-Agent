...
def parse_args():
    ...
    parser.add_argument('--tgt_path', type=str,
                        default="/home/taiping-qu/code/mr_heart_seg_thin/train/train_data/processed_data_dy_stage2")
...
def singe_generate_data(inputs):
    ...
    heart_range_crop = get_seg_max_box(heart_mask > 0, padding=40)
...
if __name__ == "__main__":
    ...
    src_dcm_dir = os.path.join(src_dir, 'CMR_dcm')
    src_seg_dir = os.path.join(src_dir, 'CMR_seg')
    src_heart_dir = os.path.join(src_dir, 'CMR_seg')
    for seg_file in sorted(os.listdir(src_seg_dir)):
        ...
        seg_file = os.path.join(src_seg_dir, pid + "-seg.nii.gz")
        heart_path = os.path.join(src_heart_dir, pid + "-seg.nii.gz")
