...
class CineClassificationPidReSampleDataset(DefaultSampleDataset):
    ...
    def _load_source_data(self, file_name):
        ...
        result = {}
        vol_2ch = torch.from_numpy(data["dcm1"]).float()
        vol_4ch = torch.from_numpy(data["dcm2"]).float()
        vol_sa  = torch.from_numpy(data["dcm3"]).float()

        # seg_2ch = torch.from_numpy(data["seg_2ch"]).float()
        # seg_4ch = torch.from_numpy(data["seg_4ch"]).float()
        # seg_sa  = torch.from_numpy(data["seg_sa"]).float()
        
        gt = data["gt"]
        
        result["vol_2ch"] = vol_2ch[None, None].detach()  # [1,1,D,H,W]
        result["vol_4ch"] = vol_4ch[None, None].detach()
        result["vol_sa"]  = vol_sa[None, None].detach()
        result["vol_shape_2ch"] = np.array(vol_2ch.size())
        result["vol_shape_4ch"] = np.array(vol_4ch.size())
        result["vol_shape_sa"]  = np.array(vol_sa.size())
        
        # result["seg_2ch"] = seg_2ch[None, None].detach()  # [1,1,D,H,W]
        # result["seg_4ch"] = seg_4ch[None, None].detach()
        # result["seg_sa"]  = seg_sa[None, None].detach()
    ...
    def _sample_source_data(self, idx, source_data_info):
        ...
        vol_2ch, vol_shape_2ch = _source_data["vol_2ch"], _source_data["vol_shape_2ch"]
        ...
        # seg_2ch = _source_data["seg_2ch"]
        # seg_4ch = _source_data["seg_4ch"]
        # seg_sa  = _source_data["seg_sa"]
        
        gt = _source_data["gt"]

        center_2ch = self._get_random_crop_center(vol_shape_2ch, "2ch")
        vol_2ch_crop = self._crop_data(vol_2ch, center_2ch, [80, 192, 192])
...
class Cine_Cls_ReSampleDataset_Val(DefaultSampleDataset):
    ...
    def _load_source_data(self, file_name):
        ...
        # Store image data
        result["vol_2ch"] = vol_2ch[None, None].detach()
        result["vol_4ch"] = vol_4ch[None, None].detach()
        result["vol_sa"]  = vol_sa[None, None].detach()
        result["vol_shape_2ch"] = np.array(vol_2ch.size())
        result["vol_shape_4ch"] = np.array(vol_4ch.size())
        ...
        # Store seg data
        # result["seg_2ch"] = seg_2ch[None, None].detach()
        # result["seg_4ch"] = seg_4ch[None, None].detach()
        # result["seg_sa"]  = seg_sa[None, None].detach()
    ...
    def _sample_source_data(self, idx, source_data_info):
        ...
        # vol = self._crop_data(vol, c_t, self._patch_size)
        ...
        center_2ch = self._get_crop_center(vol_shape_2ch, "2ch")
        vol_2ch_crop = self._crop_data(vol_2ch, center_2ch, [80, 192, 192])
