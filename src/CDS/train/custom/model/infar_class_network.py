...
class CineClassification_Network(nn.Module):
    ...
    @torch.jit.ignore
    def forward(self, img_2ch, img_4ch, img_sa, gt):
        # TODO: 定制forward网络
        aug_2ch = self._pipeline(dict(img=img_2ch.squeeze(1)))["img"]
        aug_4ch = self._pipeline(dict(img=img_4ch.squeeze(1)))["img"]
        aug_sa  = self._pipeline(dict(img=img_sa.squeeze(1)))["img"]
        
        # 多加一类
        outs_cls, deep_sup_cls,  deep_sup_cls_2ch, deep_sup_cls_4ch, deep_sup_cls_sa= self.backbone(aug_2ch, aug_4ch, aug_sa)
        gt = gt.view(gt.shape[0] * gt.shape[1])
        loss = self.head.loss((outs_cls, deep_sup_cls, deep_sup_cls_2ch, deep_sup_cls_4ch, deep_sup_cls_sa), gt)
        return loss

    @torch.jit.export
    def forward_test(self, img_2ch, img_4ch, img_sa):
        outs, _, _, _, _ = self.backbone(img_2ch, img_4ch, img_sa)
        pred = torch.softmax(outs, dim=1)
        return pred

    def single_test(self, img_2ch, img_4ch, img_sa, gt):
        # img = img.squeeze(1)
        img_2ch = img_2ch.squeeze(1)
        img_4ch = img_4ch.squeeze(1)
        img_sa  = img_sa.squeeze(1)
        pred = self.forward_test(img_2ch, img_4ch, img_sa)
        return pred, gt
