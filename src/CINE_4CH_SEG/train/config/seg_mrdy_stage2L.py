patch_size = [32, 64, 64]
patch_size_inner = [24, 48, 48]
isotropy_spacing = [0.1, 1, 1]
...
data = dict(
    ...
    train=dict(
        ...
        dst_list_file='./data/processed_data/processed_4CH_dy_stage2_L/train.lst',
        data_root="./data/processed_data/processed_4CH_dy_stage2_L",
...
lr_config = dict(policy='step', warmup='linear', warmup_iters=50, warmup_ratio=1.0 / 3, step=[10, 50], gamma=0.2)
...
work_dir = './checkpoints/4CH_dy_second2_L'
...
load_from = './checkpoints/4CH_dy_second2_L/latest.pth'
workflow = [('train', 1)]
