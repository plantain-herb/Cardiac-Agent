...
class Color3D_Sample_Dataset(DefaultSampleDataset):
    ...
    def _load_source_data(self, file_name):
        ...
        result = {}
        vol = torch.from_numpy(data["vol"].astype(np.float32)).float()
        # vol /= 255.0
        spacing = data["spacing"].copy()
        mask = (data["mask"]).astype('uint8')
        mask = torch.from_numpy(mask).float()
        bin_mask = (data["bin_mask"]).astype('uint8')
        bin_mask = torch.from_numpy(bin_mask).float()


        roi_range = torch.from_numpy(data["sample_region"]).float()

        result["vol"] = vol[None, None].detach()
        result["mask"] = mask[None, None].detach()
        result["bin_mask"] = bin_mask[None, None].detach()

        result["spacing"] = spacing
        result["vol_shape"] = np.array(vol.size())
        result["heart_box"] = roi_range
        del data
    ...
    def _sample_source_data(self, idx, source_data_info):
        ...
        vol, mask, bin_mask, src_spacing, vol_shape,  = (
            _source_data["vol"],
            _source_data["mask"],
            _source_data["bin_mask"],
            _source_data["spacing"],
            _source_data["vol_shape"],
        )
        src_spacing[0] = src_spacing[0] * 8
        zmin, ymin, xmin, zmax, ymax, xmax = _source_data["heart_box"]
        ...
        y_width = (ymax - ymin)
        x_width = (xmax - xmin)
        heart_box = np.array([z_center, y_center, x_center, z_width, y_width, x_width])

        c_t = self._get_random_crop_center(heart_box, src_spacing)
        vol, mask_patch, bin_mask = self._crop_data(
            vol, mask, bin_mask, c_t, vol_shape, src_spacing, heart_box,
        )
        # print(zmin, ymin, xmin, zmax, ymax, xmax,z_center,c_t)
        bright_param = self._bright_aug_param_generate()
        vol = self._bright_aug_apply(vol[0], bin_mask[0], bright_param)

        vol_patch = torch.cat((vol[0], bin_mask), dim=0)
        results = {}
        results['img'] = vol_patch
        results['mask'] = mask_patch

        # import time
        # s = str(time.time())
        # print(vol.shape)
        # sitk.WriteImage(sitk.GetImageFromArray(vol[0][0].cpu().float().numpy()), "./"+ s + ".nii.gz")
        # sitk.WriteImage(sitk.GetImageFromArray(mask_patch[0].cpu().float().numpy()), "./"+ s + "-mask.nii.gz")
        return results

    def _get_random_crop_center(self, heart_box, src_spacing):
        heart_center = heart_box[:3] * src_spacing
        return np.array(heart_center)

    def _crop_data(self, vol, mask, bin_mask, c_t, src_shape, src_spacing, heart_box):
        ...
        heart_width = heart_box[3:]
        tgt_spacing = (heart_width * src_spacing) / self._patch_size
        # print(tgt_spacing, src_spacing)
        tgt_spacing = np.max(tgt_spacing)
        ...
        vol = torch.nn.functional.grid_sample(vol, grid, mode="bilinear", align_corners=True)[0]
        mask = torch.nn.functional.grid_sample(mask, grid, mode="nearest", align_corners=True)[0]
        bin_mask = torch.nn.functional.grid_sample(bin_mask, grid, mode="nearest", align_corners=True)[0]
        vol = self._normalization(vol)
        return vol, mask, bin_mask
