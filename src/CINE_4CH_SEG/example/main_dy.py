...
def parse_args():
    ...
    # parser.add_argument("--input_path", default=str(DATA_ROOT / "CINE_4CH_SEG" / "input"), type=str)
    # parser.add_argument("--output_path", default=str(OUTPUT_ROOT / "CINE_4CH_SEG"), type=str)
    parser.add_argument("--input_path", default=str(DATA_ROOT / "CINE_4CH_SEG" / "input"), type=str)
    parser.add_argument("--output_path", default=str(OUTPUT_ROOT / "CINE_4CH_SEG"), type=str)
    parser.add_argument("--model_path", default=None, type=str)
    parser.add_argument("--model_file_DY_first", default=str(WEIGHTS_ROOT / "cine_seg_first_4CH" / "Cine_4CH_seg1.pth"), type=str)
    parser.add_argument("--model_file_DY_secondL", default=str(WEIGHTS_ROOT / "cine_seg_second_4CH_L" / "Cine_4CH_seg2_L.pth"), type=str)
    parser.add_argument("--model_file_DY_secondR", default=str(WEIGHTS_ROOT / "cine_seg_second_4CH_R" / "Cine_4CH_seg2_R.pth"), type=str)
    parser.add_argument("--network_file_DY_first", default=str(MODULE_ROOT / "train" / "config" / "seg_mrdy_stage1.py"), type=str)
    parser.add_argument("--network_file_DY_secondL", type=str, default=str(MODULE_ROOT / "train" / "config" / "seg_mrdy_stage2L.py"))
    parser.add_argument("--network_file_DY_secondR", type=str, default=str(MODULE_ROOT / "train" / "config" / "seg_mrdy_stage2R.py"))
...
def main(input_path, output_path, gpu, args):
    if (
        ...
        and args.model_file_DY_secondL is not None
        and args.network_file_DY_secondL is not None
        and args.model_file_DY_secondR is not None
        and args.network_file_DY_secondR is not None
        and args.config_file is not None
    ):
        model_seg_3d = CtAbdomenSegDYModel(
            model_DY_f=args.model_file_DY_first,
            model_DY_crop_f=args.model_file_DY_secondL,
            model_DY_crop_r=args.model_file_DY_secondR,
            network_DY_f=args.network_file_DY_first,
            network_DY_crop_f=args.network_file_DY_secondL,
            network_DY_crop_r=args.network_file_DY_secondR,
    ...
    for pid in tqdm(os.listdir(input_path)):
        try:
            ...
            hu_volume = sitk.GetImageFromArray(hu_volume)
            sitk.WriteImage(hu_volume, os.path.join(output_path, f"{pid}.nii.gz"))
